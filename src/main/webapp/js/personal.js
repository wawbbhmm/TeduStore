...
$(".save_recipient").click(function(){
	...
	if(receiverName && receiverState && receiverCity && receiverDistrict && receiverAddress && receiverMobile){
        // 如果文本内容为修改,则执行if语句体;否则(添加收货人地址),执行else
...
$(".man").click(function(){
	if(!$(".man").hasClass("selected")){
		...
		$(".man img").attr("src","${pageContext.request.contextPath}/images/personage/select.png");
		$(".women").removeClass("selected");
		$(".women img").attr("src","${pageContext.request.contextPath}/images/personage/un_select.png");
	}
})
...
$(".women").click(function(){
	if(!$(".women").hasClass("selected")){
		...
		$(".women img").attr("src","${pageContext.request.contextPath}/images/personage/select.png");
		$(".man").removeClass("selected");
		$(".man img").attr("src","${pageContext.request.contextPath}/images/personage/un_select.png");
	}
})
/**
 * 更改用户名，变成可编辑状态
